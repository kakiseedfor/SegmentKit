...
@interface NestCollectionView () <UIGestureRecognizerDelegate, CALayerDelegate>
...
- (void)setContentOffset:(CGPoint)contentOffset{
    ...
    if (offset > point.y) { //上拉情况
        ...
        if (offset > _originalOffset) { //偏移位置大于原始偏移
            ...
            if (self.currentPoint < _originalPoint + _scrollDelegate.restOffset) {   //当前位置小于原始位置
                [self setDelegateRemoveFix:YES];    //打开父View的滑动标志
                [super setContentOffset:CGPointMake(point.x, _originalOffset)]; //固定在原始偏移
            }else if (self.currentPoint > _originalPoint + _scrollDelegate.restOffset){  //当前位置大于原始位置
                if (_removeFix) {   //滑动标志已打开
